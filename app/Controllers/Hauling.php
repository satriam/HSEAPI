...
class Hauling extends ResourceController
{
    protected $modelName = 'App\Models\HaulingsModel';
    protected $format = 'json';

    public function index()
    {
        $data = $this->model->orderBy('id_haulings', 'asc')->findAll();
        if($data == null){
            ...
            $decoded = JWT::decode($token, $key, ['HS256']);
                ...
                $userid = $decoded->uid;
                    ...
                    if ($validate) {
                        ...
                        $file->move("haulings", $newfilename);
                    
                            $temp2 = explode(".", $img2);
                            $newfilename2 = round(microtime(true)) .'2'. '.' . end($temp2);
                            $file2->move("haulings", $newfilename2);
                       
                        $hauling = [
                            "img_url_1" => "/haulings/" . $newfilename,
                            "img_url_2" => "/haulings/" . $newfilename2,
                           ...
                           "kondisi_26"=>$data['kondisi27'],
                           "kondisi_26"=>$data['kondisi28'],
                           "kondisi_26"=>$data['kondisi29'],
                           "kondisi_26"=>$data['kondisi30'],
                           ...
                           "kode_bahaya_26"=>$data['kode27'],
                           "kode_bahaya_26"=>$data['kode28'],
                           "kode_bahaya_26"=>$data['kode29'],
                           "kode_bahaya_26"=>$data['kode30'],
                           "tanggal"=>$data['tanggal'],
                            "nama_lokasi"=>$data['namalokasi'],
                            "nama_hauling"=>$data['namahauling'],
                            "shift"=>$data['shift'],
                            "grup"=>$data['grup'],
                        ...
                        $builder = $db->table('haulings');
                        $builder->insert($hauling);
    ...
    public function delete($id = null)
    {
        $db = \Config\Database::connect();
        //cek lokasi
        $usersTable = $db->table("haulings");
        $usersTable->select("*");
        $usersTable->where('id_haulings', $id);
    ...
    $decoded = \Firebase\JWT\JWT::decode($token, $key, ['HS256']);
        ...
        $builder = $db->table('haulings');
        $builder->select('id_haulings,nama_pengawas,img_url_1,nama_lokasi,haulings.created_at');
        $builder->join('users', 'users.user_id=haulings.created_by_id');
